...
for i in range(0, len(speed_data) - 2*SEQUENCE_LEN - 1):
    X_data.append(speed_data[(i + SEQUENCE_LEN)-1])
    Y_data.append(wind_speed_data[(i + SEQUENCE_LEN)])
...
def forward_fill(x):
    ret = x[1:]
    ret = torch.cat((ret, torch.zeros(1, len(ret[0])).to(ret.device)), 0)
    return ret

def forecast_batch(model, x, steps):
    ret = []
    ret.append(torch.cat([model(x), forward_fill(x[:,1:])], 1))
    for i in range(steps - 1):
        ret.append(torch.cat([model(ret[-1]), forward_fill(ret[-1][:,1:])], 1))
    return torch.stack(ret, 1)
        
y_forecast = forecast_batch(model, xval.cuda(), 12)
# Drop env variables
y_forecast = y_forecast[:,:,0]

true_forecast = torch.zeros(len(yval) - (12 - 1), 12).to(yval.device)

for i in range(12):
    true_forecast[:,i] = yval[i:len(yval) - (12 - 1)+i].squeeze()

y_forecast = y_forecast[:len(true_forecast)]

err = (y_forecast - true_forecast)**2
